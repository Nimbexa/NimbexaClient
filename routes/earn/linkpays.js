...
module.exports.load = async function (app, db) {
  ...
  app.get(`/earn/linkpays/generate`, async (req, res) => {
    ...
    try {
      const response = await fetch(`https://linkpays.in/api?api=${settings.linkpays.apiKey}&url=${encodeURIComponent(link)}&alias=Nimbexa${alias}`);
  ...
  app.get(`/earn/linkpays/redeem/:code`, async (req, res) => {
    ...
    const usercode = lpcodes[req.session.userinfo.email];
    ...
    await db.set(`dailylinkpays-${req.session.userinfo.email}`, 1);
    const coins = await db.get(`coins-${req.session.userinfo.email}`)
    await db.set(`coins-${req.session.userinfo.email}`, coins + settings.linkpays.coins)    
